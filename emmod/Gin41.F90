SUBROUTINE gin41 (Gin, zrcv, zsrc, etaH, etaV, zetaH, zetaV, gamA, gamB, xpos, x, y, nlogx)
...
do ix=1,nlogx
	...
	mult = x(ix)*(zrcv-zsrc)/(4.0*pi*sqrt(etaH*zetaH));
	afac = sqrt(gamA)*etaH/(RbigA*etaV);
	bfac = sqrt(gamB)*zetaH/(RbigB*zetaV);
	term1 = afac*mult*exp(-1.0*sqrt(gamA)*RbigA)*(2.0*y(ix)*rad**(-4.0)+sqrt(gamA)*y(ix)/(rad**2.0*RbigA)+y(ix)/(rad**2.0*RbigA**2.0));
	term2 = bfac*mult*exp(-1.0*sqrt(gamB)*RbigB)*(2.0*y(ix)*rad**(-4.0)+sqrt(gamB)*y(ix)/(rad**2.0*RbigB)+y(ix)/(rad**2.0*RbigB**2.0));
	Gin(ix) = term1 - term2;
end do

END SUBROUTINE gin41
