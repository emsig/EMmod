SUBROUTINE gridit_yxm (Ptot,xtotrad1,xtotrad2,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,above,xdirect)
! Compute the data of one quadrant of the grid based on the radial data using pchip interpolation
! for component 51 (crossline oriented magnetic receiver, inline oriented electric source).
! 
! Calling arguments:
...
do iy=1,nyh
	...
	do ix=1,nxh
		...
		if (above==0 .AND. xdirect==1) then
			CALL gin51(gin,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,rad,x,y,1);
			Ptot(ix,iy) = Ptot(ix,iy) + gin;
		endif
	end do
end do
END SUBROUTINE gridit_yxm

SUBROUTINE gridit_yxm_lin (Ptot,xtotrad1,xtotrad2,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,above,xdirect)
...
END SUBROUTINE gridit_yxm_lin

SUBROUTINE gridit_yxm_fullspace (Ptot,dx,nxh,dy,nyh,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB)
...
END SUBROUTINE gridit_yxm_fullspace

SUBROUTINE gridit_yym (Ptot,xtotrad1,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,above,xdirect)
! Compute the data of one quadrant of the grid based on the radial data using pchip interpolation
! for component 52 (crossline oriented magnetic receiver, crossline oriented electric source).
! 
! Calling arguments:
! Ptot:     After completion of this function, this array will contain one quadrant of the EM-field in the space domain, 
!           complex array of size nxh times nyh
...
do iy=1,nyh
	...
	do ix=1,nxh
		...
		if (above==0 .AND. xdirect==1) then
			CALL gin52(gin,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,rad,x,y,1);
			Ptot(ix,iy) = Ptot(ix,iy) + gin;
		endif
	end do
end do
END SUBROUTINE gridit_yym

SUBROUTINE gridit_yym_lin (Ptot,xtotrad,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,above,xdirect)
! Compute the data of one quadrant of the grid based on the radial data using linear interpolation
! for component 52 (crossline oriented magnetic receiver, crossline oriented electric source).
! 
! Calling arguments:
...
do iy=1,nyh
	...
	do ix=1,nxh
		...
		Ptot(ix,iy) = fact*(weightlo*xtotrad(lopos)+weighthi*xtotrad(hipos));
		if (above==0 .AND. xdirect==1) then
			CALL gin52(gin,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,rad,x,y,1);
			Ptot(ix,iy) = Ptot(ix,iy) + gin;
		endif
...
END SUBROUTINE gridit_yym_lin

SUBROUTINE gridit_yym_fullspace (Ptot,dx,nxh,dy,nyh,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB)
! Compute the data of one quadrant of the grid for a homogeneous fullspace
! for component 52 (crossline oriented magnetic receiver, crossline oriented electric source).
! 
! Calling arguments:
...
END SUBROUTINE gridit_yym_fullspace

SUBROUTINE gridit_yzm (Ptot,xtotrad1,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,gamA,above,xdirect)
! Compute the data of one quadrant of the grid based on the radial data using pchip interpolation
! for component 23 (crossline oriented magnetic receiver, vertical electric source).
! 
! Calling arguments:
! Ptot:     After completion of this function, this array will contain one quadrant of the EM-field in the space domain, 
!           complex array of size nxh times nyh
...
do iy=1,nyh
	...
	do ix=1,nxh
		...
		if (above==0 .AND. xdirect==1) then
			CALL gin53(gin,zrcv,zsrc,etaH,etaV,zetaH,gamA,rad,x,1);
			Ptot(ix,iy) = Ptot(ix,iy) - gin;
		endif
	end do
end do
END SUBROUTINE gridit_yzm

SUBROUTINE gridit_yzm_lin (Ptot,xtotrad1,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,gamA,above,xdirect)
! Compute the data of one quadrant of the grid based on the radial data using linear interpolation
! for component 53 (crossline oriented magnetic receiver, vertical electric source).
! 
! Calling arguments:
! Ptot:     After completion of this function, this array will contain one quadrant of the EM-field in the space domain, 
!           complex array of size nxh times nyh
...
END SUBROUTINE gridit_yzm_lin

SUBROUTINE gridit_yzm_fullspace (Ptot,dx,nxh,dy,nyh,zrcv,zsrc,etaH,etaV,zetaH,gamA)
! Compute the data of one quadrant of the grid for a homogeneous fullspace
! for component 53 (crossline oriented magnetic receiver, vertical electric source).
! 
! Calling arguments:
...
END SUBROUTINE gridit_yzm_fullspace
