SUBROUTINE Ptotalxxm (Ptot, Pdownplus, Pupplus, Pdownminbar, Pupminbar, Wup, Wdown, Wupbar, Wdownbar, &
	Gam, GammaB, Rp, Rpbar, Rm, Rmbar, etaH, etaV, zetaH, zetaV, nk, cor, nz, z, zrcv, zsrc, &
	zsrclay, zrcvlay, gamA, gamB, nlayers, above, xdirect);
! Calculate total field for the 41 component (inline oriented magnetic receiver, inline oriented electric source)
!
! Calling arguments:
...
if (above == 0) then ! receivers in the source layer
	do ikx=1,nk
		if (zsrclayfort == 1) then ! If the source and the receivers are in the top most layer
			Ptot(ikx) = Pupplus(ikx)*Wup(ikx)+Pupminbar(ikx)*Wupbar(ikx);
			if (xdirect /= 1) then ! The direct field is NOT computed in the space domain
				Ptot(ikx) = Ptot(ikx) - signd*exp(-1.0*Gam(ikx,zsrclayfort)*d) &
		...
		elseif (zsrclayfort == nz) then ! If the source and the receivers are in the lower most layer
			Ptot(ikx) = -1.0*Pdownplus(ikx)*Wdown(ikx)-Pdownminbar(ikx)*Wdownbar(ikx);
			if (xdirect /= 1) then ! The direct field is NOT computed in the space domain
				Ptot(ikx) = Ptot(ikx) - signd*exp(-1.0*Gam(ikx,zsrclayfort)*d) &
					&+ signd*exp(-1.0*GammaB(ikx,zsrclayfort)*d);
			endif
		else ! If the source and the receivers are in any layer in between. 
			Ptot(ikx) = Pupplus(ikx)*Wup(ikx)-Pdownplus(ikx)*Wdown(ikx);
			Ptot(ikx) = Ptot(ikx) + Pupminbar(ikx)*Wupbar(ikx)-Pdownminbar(ikx)*Wdownbar(ikx);
			if (xdirect /= 1) then ! The direct field is NOT computed in the space domain
				Ptot(ikx) = Ptot(ikx) - signd*exp(-1.0*Gam(ikx,zsrclayfort)*d) &
					&+ signd*exp(-1.0*GammaB(ikx,zsrclayfort)*d);
			endif
...
END SUBROUTINE Ptotalxxm

SUBROUTINE Ptotalxym (Ptot1,Ptot2, Pdownplus, Pupplus, Pdownminbar, Pupminbar, Wup, Wdown, Wupbar, Wdownbar, &
	Gam, GammaB, Rp, Rpbar, Rm, Rmbar, etaH, etaV, zetaH, zetaV, nk, cor, nz, z, zrcv, zsrc, &
	zsrclay, zrcvlay, gamA, gamB, nlayers, above, xdirect);
! Calculate total field for the 42 component (inline oriented magnetic receiver, crossline oriented electric source)
!
! Calling arguments:
...
END SUBROUTINE Ptotalxym

SUBROUTINE Ptotalxzm (Ptot, Pdownplus, Pupplus, Wup, Wdown, &
	Gam, Rp, Rm, etaH, etaV, zetaH, zetaV, nk, cor, nz, z, zrcv, zsrc, &
	zsrclay, zrcvlay, gamA, gamB, nlayers, above, xdirect);
! Calculate total field for the 43 component (inline oriented magnetic receiver, vertical electric source)
!
! Calling arguments:
! Ptot:        Total field, complex array of size nk
! Pdownplus:   TM-mode part of the downgoing field, complex array of size nk
